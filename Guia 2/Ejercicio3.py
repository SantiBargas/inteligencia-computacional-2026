...
def graficar_historial(historial_xi, historial_error_clasif, etiqueta=""):
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(13, 5))
    titulo = "Evolución del entrenamiento por época"
    if etiqueta:
        titulo += f" — arquitectura {etiqueta}"
    fig.suptitle(titulo, fontsize=13)

    ax1.plot(historial_xi, marker='.', markersize=3, label=etiqueta)
    ax1.set_xlabel('Época')
    ax1.set_ylabel('ξ = ½·Σ e² (suma sobre todos los patrones)')
    ax1.set_title('Error cuadrático total\n(lo que minimiza el gradiente descendente)')
...
def Ejercicio3(capas, ruta_train, ruta_test, vel_aprendizaje, max_epocas, err_umbral, activacion):
    ...
    historial_error = []  # Lista para almacenar el error de cada época
    historial_error_clasif=[]  # Lista para almacenar el error de clasificación de cada época
    ...
    for epoca in range(max_epocas):
        ...
        historial_error.append(error_epoca)  # Guardamos el error de esta época
        historial_error_clasif.append(1 - tasa_aciertos)  # Guardamos el error de clasificación de esta época
